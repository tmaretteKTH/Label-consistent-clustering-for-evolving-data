...
from algorithms.carv import Carvrstarng
...
def greedyAndProject(
    ...
    while len(S) > k and nbTries < 5:
        # we try a few times to get a clustering with Carv
        S = Carvrstarng(points, rStar, k, rng)  # cover points with radius 2r^*
        if len(S) == 1 and k > 3:
            S = list(range(k + 1))
        nbTries += 1
    if len(S) > k:
